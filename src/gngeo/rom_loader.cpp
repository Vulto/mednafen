...
{
    if(roms == nullptr || roms->tiles.p == nullptr)
        return false;

    Uint32 usage_size =
        (roms->tiles.size >> 11) * sizeof(Uint32);

    if(GnGeoAllocateRegion(
        &roms->spr_usage,
        usage_size,
        REGION_SPR_USAGE) != 0)
    ...
    {
        ((Uint32 *)roms->spr_usage.p)[i >> 4] |=
            GnGeoConvertRomTile(roms->tiles.p, i);
    }
...
{
    if(roms == nullptr ||
       roms->game_sfix.p == nullptr ||
       roms->gfix_usage.p == nullptr)
        return false;
                            ...
                            const char *filename)
{
    ...
    if(src != 0)
    {
        try
        {
            if(region == REGION_SPRITES)
                stream->seek(src / 2, SEEK_SET);
            else
                stream->seek(src, SEEK_SET);
        }
        catch(const Mednafen::MDFN_Error&)
        {
            return false;
    ...
    switch(region)
    {
        ...
        case REGION_MAIN_CPU_BIOS:
            target = &roms->bios_m68k;
            break;

        case REGION_AUDIO_CPU_BIOS:
            target = &roms->bios_audio;
            break;

        case REGION_FIXED_LAYER_BIOS:
            target = &roms->bios_sfix;
            break;
    ...
    if(region == REGION_SPRITES)
    {
        if(target->size < (dest & ~1U) + size * 2U)
            return false;
...
static bool GnGeoLoadBios(
    Mednafen::GameFile *gf,
    GAME_ROMS *roms,
    SYSTEM system,
    COUNTRY country)
{
    ...
    if(roms->bios_sfix.p == nullptr)
    {
        ...
        try
        {
            stream.reset(bios_archive->open(
                "sfix.sfx",
        ...
        if(GnGeoAllocateRegion(
            &roms->bios_sfix,
            size,
            REGION_FIXED_LAYER_BIOS) != 0)
            return false;

        if(stream->read(roms->bios_sfix.p, size) != size)
            return false;
    }
    ...
    if(roms->bios_m68k.p == nullptr)
    {
        ...
        if(GnGeoAllocateRegion(
            &roms->bios_m68k,
            size,
            REGION_MAIN_CPU_BIOS) != 0)
            return false;

        if(stream->read(roms->bios_m68k.p, size) != size)
            return false;
    }
...
bool Mednafen::GnGeoLoadRomSet(Mednafen::GameFile *gf, GAME_ROMS *roms, SYSTEM system, COUNTRY country)
{
    ...
    std::unique_ptr<Mednafen::Stream> drv(
        data_archive->open(
            drv_path,
            Mednafen::VirtualFS::MODE_READ));
    ...
    for(unsigned i = 0; i < 10; i++)
        drv->read(
            &drv_def.romsize[i],
            sizeof(drv_def.romsize[i]));

    drv->read(
        &drv_def.nb_romfile,
        sizeof(drv_def.nb_romfile));

    if(drv_def.nb_romfile > 32)
        return false;

    for(unsigned i = 0; i < drv_def.nb_romfile; i++)
    {
        drv->read(
            drv_def.rom[i].filename,
            sizeof(drv_def.rom[i].filename));

        drv->read(
            &drv_def.rom[i].region,
            sizeof(drv_def.rom[i].region));

        drv->read(
            &drv_def.rom[i].src,
            sizeof(drv_def.rom[i].src));

        drv->read(
            &drv_def.rom[i].dest,
            sizeof(drv_def.rom[i].dest));

        drv->read(
            &drv_def.rom[i].size,
            sizeof(drv_def.rom[i].size));

        drv->read(
            &drv_def.rom[i].crc,
            sizeof(drv_def.rom[i].crc));
    }
    ...
    if(GnGeoAllocateRegion(
        &roms->cpu_m68k,
        drv_def.romsize[REGION_MAIN_CPU_CARTRIDGE],
        REGION_MAIN_CPU_CARTRIDGE) != 0)
        { GnGeoFreeRoms(roms); return false; }

    if(drv_def.romsize[REGION_AUDIO_CPU_CARTRIDGE] == 0 &&
       drv_def.romsize[REGION_AUDIO_CPU_ENCRYPTED] != 0)
    {
        if(GnGeoAllocateRegion(
            &roms->cpu_z80c,
            0x80000,
            REGION_AUDIO_CPU_ENCRYPTED) != 0 ||
           GnGeoAllocateRegion(
            &roms->cpu_z80,
            0x90000,
            REGION_AUDIO_CPU_CARTRIDGE) != 0)
            { GnGeoFreeRoms(roms); return false; }
    }
    else if(GnGeoAllocateRegion(
        &roms->cpu_z80,
        drv_def.romsize[REGION_AUDIO_CPU_CARTRIDGE],
        REGION_AUDIO_CPU_CARTRIDGE) != 0)
        { GnGeoFreeRoms(roms); return false; }

    if(GnGeoAllocateRegion(
        &roms->tiles,
        drv_def.romsize[REGION_SPRITES],
        REGION_SPRITES) != 0)
        { GnGeoFreeRoms(roms); return false; }

    if(GnGeoAllocateRegion(
        &roms->game_sfix,
        drv_def.romsize[REGION_FIXED_LAYER_CARTRIDGE],
        REGION_FIXED_LAYER_CARTRIDGE) != 0)
        { GnGeoFreeRoms(roms); return false; }

    if(GnGeoAllocateRegion(
        &roms->gfix_usage,
        roms->game_sfix.size >> 5,
        REGION_GAME_FIX_USAGE) != 0)
        { GnGeoFreeRoms(roms); return false; }

    if(GnGeoAllocateRegion(
        &roms->adpcma,
        drv_def.romsize[REGION_AUDIO_DATA_1],
        REGION_AUDIO_DATA_1) != 0)
        { GnGeoFreeRoms(roms); return false; }

    if(GnGeoAllocateRegion(
        &roms->adpcmb,
        drv_def.romsize[REGION_AUDIO_DATA_2],
        REGION_AUDIO_DATA_2) != 0)
        { GnGeoFreeRoms(roms); return false; }

    if(drv_def.romsize[REGION_MAIN_CPU_BIOS] != 0)
    {
        roms->info.flags |= HAS_CUSTOM_CPU_BIOS;

        if(GnGeoAllocateRegion(
            &roms->bios_m68k,
            drv_def.romsize[REGION_MAIN_CPU_BIOS],
            REGION_MAIN_CPU_BIOS) != 0)
            { GnGeoFreeRoms(roms); return false; }
    }

    if(drv_def.romsize[REGION_AUDIO_CPU_BIOS] != 0)
    {
        roms->info.flags |= HAS_CUSTOM_AUDIO_BIOS;

        if(GnGeoAllocateRegion(
            &roms->bios_audio,
            drv_def.romsize[REGION_AUDIO_CPU_BIOS],
            REGION_AUDIO_CPU_BIOS) != 0)
            { GnGeoFreeRoms(roms); return false; }
    }

    if(drv_def.romsize[REGION_FIXED_LAYER_BIOS] != 0)
    {
        roms->info.flags |= HAS_CUSTOM_SFIX_BIOS;

        if(GnGeoAllocateRegion(
            &roms->bios_sfix,
            drv_def.romsize[REGION_FIXED_LAYER_BIOS],
            REGION_FIXED_LAYER_BIOS) != 0)
            { GnGeoFreeRoms(roms); return false; }
    }

    if(!GnGeoLoadBios(gf, roms, system, country))
        { GnGeoFreeRoms(roms); return false; }
    ...
    if(gf->outside.dir.empty())
        game_path = gf->outside.fbase + ".zip";
    else
        game_path = gf->outside.dir + "/" + gf->outside.fbase + ".zip";
    ...
    if(drv_def.parent[0] != 0)
    {
        ...
        if(gf->outside.dir.empty())
            parent_path =
                std::string(drv_def.parent) + ".zip";
        else
            parent_path =
                gf->outside.dir + "/" +
                std::string(drv_def.parent) +
                ".zip";

        parent_archive.reset(
            Mednafen::ArchiveReader::Open(
                gf->outside.vfs,
                parent_path));
    }
    ...
    for(unsigned i = 0; i < drv_def.nb_romfile; i++)
    {
        ...
        if(rom.region != REGION_FIXED_LAYER_BIOS &&
           rom.region != REGION_AUDIO_CPU_BIOS &&
           rom.region != REGION_MAIN_CPU_BIOS)
            { GnGeoFreeRoms(roms); return false; }
    }

    if(roms->adpcmb.size == 0)
    {
        roms->adpcmb.p = roms->adpcma.p;
        roms->adpcmb.size = roms->adpcma.size;
    }

    if(!GnGeoConvertAllTile(roms))
        { GnGeoFreeRoms(roms); return false; }

    if(!GnGeoConvertAllChar(roms))
        { GnGeoFreeRoms(roms); return false; }
