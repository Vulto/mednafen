...
if [ "$#" -ne 1 ]; then
    echo "Usage: $0 /path/to/kof98.zip" >&2
    exit 2
fi
...
TestDir="${TMPDIR:-/tmp}/mednafen-gngeo-kof98"
rm -rf "$TestDir"
mkdir -p "$TestDir"
...
if [ "$ReturnCode" -ne 124 ]; then
    echo "KOF98 runtime test failed: Mednafen exited before the 30 second acceptance window (rc=$ReturnCode)." >&2
    exit 1
fi
...
echo "KOF98 remained running in the Mednafen GnGeo core for 30 seconds."
